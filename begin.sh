...
if [ -f "${CURRENT_DIR}/output1.log" ]; then
  if [ -f "${CURRENT_DIR}/output2.log" ]; then
    if [ -f "${CURRENT_DIR}/output3.log" ]; then
      ...
      nohup python3 -u CreatInstance.py > output4.log 2>&1 & echo "$!" >> pid4.txt && echo "$shape_name" >> pid4.txt
      exit 0
    fi
    nohup python3 -u CreatInstance.py > output3.log 2>&1 & echo "$!" >> pid3.txt && echo "$shape_name" >> pid3.txt
    exit 0
  fi
  nohup python3 -u CreatInstance.py > output2.log 2>&1 & echo "$!" >> pid2.txt && echo "$shape_name" >> pid2.txt
  exit 0
fi
nohup python3 -u CreatInstance.py > output1.log 2>&1 & echo "$!" >> pid1.txt && echo "$shape_name" >> pid1.txt
exit 0
